      ...
      <nav className="flex flex-1 flex-col gap-1 p-3" aria-label="Main">
        {navItems.map((item) => (
          <NavLink
            ...
            to={item.to}
            className={({ isActive }) =>
              `group relative flex items-center gap-2 rounded-[var(--radius-btn)] px-3 py-2 text-sm font-medium transition-colors duration-[var(--dur-fast)] ${
                isActive ? 'bg-accent-soft text-accent' : 'text-ink-2 hover:bg-paper-3 hover:text-ink'
              }`
            }
          >
            {({ isActive }) => (
